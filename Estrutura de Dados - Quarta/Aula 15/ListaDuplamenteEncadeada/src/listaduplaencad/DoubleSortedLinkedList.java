...
class DoubleSortedLinkedList<E> {
    ...
    private Node head, tail; //Nós do começo da Lista Ordenada
    ...
    public Object remove(Comparable x) {    // O(n)
        ...
        //Se elemento nulo, lista vazia ou o elemento for menor que o primeiro (nÃ£o estÃ¡ na lista):
        if (x == null || isEmpty() || x.compareTo(getHead().getValue()) < 0) {
            ...
            if (x.compareTo(getHead().getValue()) == 0) { //se for o primeiro elemento	
                ...
                if (getHead() == null) {
                    tail = null; //porque foi eliminado o Ãºnico
                } else {
                    head.setPrev(null);
                }
                temp.setNext(null);
                ...
                if (depois != null && x.compareTo(depois.getValue()) == 0) {
                    ...
                    if (antes.getNext() == null) {
                        tail = antes; //porque foi eliminado o Ãºltimo
                    }
                    if (depois.getNext() != null) {
                        depois.getNext().setPrev(antes);
                    }
                    depois.setNext(null);
                    depois = null;  //liberamos memÃ³ria
                    return x;
                } else {
    ...
    public void clear() {     // O(n)
        ...
        while (aux != null) {
            Node temp = aux;
            aux = aux.getNext(); //avança o ponteiro aux
            temp.setNext(null);
            temp = null; //liberar memória
        }
        head = null;
    }
    ...
    //Retorna o conteúdo da lista duplamente ligada, em ordem reversa
