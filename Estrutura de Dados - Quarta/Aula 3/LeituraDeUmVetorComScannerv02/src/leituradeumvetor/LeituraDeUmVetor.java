...
public class LeituraDeUmVetor {
    ...
    public LeituraDeUmVetor() {   //método construtor da classe principal
        ...
        vetor = new double[N];  //alocamos (reservamos) memória para N valores reais
        ...
        double maior = vetor[0];  //primeiro elemento, suposição inicial
        for (int i = 1; i < N; i++) { //o ciclo começa no segundo elemento (posição 1)
            if(vetor[i] > maior) maior = vetor[i];
        }        
        System.out.println("\nO maior valor guardado neste vetor é: " + maior);
